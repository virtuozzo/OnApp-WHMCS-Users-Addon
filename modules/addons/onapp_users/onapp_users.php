...
function onapp_users_output( $vars ) {
	...
	if( ! file_exists( $smarty->template_dir ) ) {
		$msg = 'Copy folder ' . ROOTDIR . '/' . $customadminpath . '/templates/v4/onapp_users_addon to ' . ROOTDIR . '/' . $customadminpath . '/templates/' . $GLOBALS[ 'aInt' ]->adminTemplate . '/';
		exit( $msg );
	}
	...
	$smarty->assign( 'pages', isset( $data[ 'pages' ] ) ? $data[ 'pages' ] : '' );
	$smarty->assign( 'current', isset( $data[ 'current' ] ) ? $data[ 'current' ] : '' );
	...
	$smarty->assign( 'total', isset( $data[ 'total' ] ) ? $data[ 'total' ] : '' );
